...
"""def insertOrUpdate(Id,Name):
    ...
    cmd="select * from mydatabase where ID="+str(Id)
    cursor=conn.execute(cmd)
    isRecordExist=0
    for row in cursor:
        isRecordExist=1
    if(isRecordExist==1):
        cmd="update mydatabase set Name="+str(Name)+"where ID="+str(Id)
    else:
        cmd="insert into mydatabase(ID,Name) values("+str(Id)+","+str(Name)+")"
    conn.execute(cmd)
    conn.commit()
    conn.close()
"""
